...
var Preloader = new Class({
    ...
    _onLoadError: function (url, error) {
        console.warn('Couldn\t load ' + url + ' received error: ' + error);

        var currPerc = this.percentageOfLoad[url];
    ...
    _onLoadProgress: function (url, progress) {
        ...
        this.onProgress.emit('progress', this.percTotal + curPerc, this.urlLoading);
    ...
    /**
     * Returns true / false depending on if all urls are finished loading or not
     *
     * @method _checkComplete
     * @returns {boolean} Is loading done?
     * @private
     */
