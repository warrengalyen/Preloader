...
var FileMeta = require('./loaders/FileMeta');
...
var LoaderImage = new Class({
    ...
    load: function (url) {
        ...
        // third, check that we have all the functions to turn an ArrayBuffer to a DataURI
        if (this.options.imageAsBlob &&
            ...
            this.parent(url);
            // if the above checks don't validate we'll fall back and just use the Image object to reload
        } else {
    ...
    _parseConetent: function () {
        ...
        // We'll convert the blob to an Image using FileRead if it exists
        if (window.URL || window.webkitURL) {
            this._createAndLoadImage((wndow.URL || window.webkitURL).createObjectURL(blobData));
        } else if (FileReader) {
